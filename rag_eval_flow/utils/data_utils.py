...
def sample_dataframe(sample_size : int, df: pd.DataFrame, random_seed: int = 42) -> pd.DataFrame:
...
def find_latest_metric_files(directory_path: str) -> list[Path]:
    """
...
def construct_one_model_metrics_df(filepath_list: list[Path]) -> tuple[pd.DataFrame, set]:
    """
...
def construct_one_metric_df(file_path : Path) -> tuple[pd.DataFrame, str]:
    try:
        ...
        return_slice = data_df.rename(columns={
...
def _legacy_construct_one_metric_df(file_path : Path) -> tuple[pd.DataFrame, str]:
    ...
    renamed_ev_data = ev_data.rename(columns={
        'score': f'{metric_name}_score',
        'explanation': f'{metric_name}_explanation'
    })
